...
class GetTrendingSongsUsecase(private val userRepository: UserRepository) {
    fun call(commonRequestModel: CommonRequestModel): Flow<Resource<SongResponseModel>> = flow {
        emit(Resource.Loading())
        try {
            val response = userRepository.getTrendingSongs(commonRequestModel)
            Log.d(LogTag.TSONGS, "API Response, ${response.message}")
            Log.d(LogTag.TSONGS, "API Response, ${response.data}")
            emit(response)
        } catch (e: HttpException) {
            Log.d(LogTag.TSONGS, e.localizedMessage!!)
